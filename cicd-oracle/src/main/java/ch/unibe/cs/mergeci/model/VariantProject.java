            ...
            if (cls.getMergeBlocks() != null) {
                for (IMergeBlock block : cls.getMergeBlocks()) {
                    if (block instanceof FixedTextBlock ftb) {
                        // Flattened manual block covers N original ConflictBlocks
                        for (int i = 0; i < ftb.getReplacedConflictBlockCount(); i++) {
                            patternNames.add("MANUAL");
                        }
                    } else if (block instanceof ConflictBlock conflict) {
