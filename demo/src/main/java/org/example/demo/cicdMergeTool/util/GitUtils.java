package org.example.demo.cicdMergeTool.util;

import org.example.demo.cicdMergeTool.model.Project;
import org.example.demo.cicdMergeTool.util.model.MergeInfo;
...
public class GitUtils {
    ...
    public static Map<String, ObjectId> getNonConflictObjects(Git git, ObjectId commit1, ObjectId commit2) throws IOException {
...
//                        System.out.println("Auto-merged: " + path + "\t" + treeWalk.getObjectId(0).getName());
    ...
    public static Map<String, ObjectId> getNonConflictObjectsFromCurrentMerge(Git git) throws IOException {
        Map<String, ObjectId> nonConflictObjects = new HashMap<>();
        Repository repo = git.getRepository();
        ObjectId oursBranch = repo.resolve("HEAD");
        ObjectId theirsBranch = repo.resolve("MERGE_HEAD");

        try (RevWalk walk = new RevWalk(repo)) {
            RevCommit head = walk.parseCommit(oursBranch);


            // Conflicting files
            ResolveMerger resolveMerger = makeMerge(oursBranch.getName(), theirsBranch.getName(), git);

            Set<String> conflictPaths = resolveMerger.getMergeResults().keySet();
            System.out.println("Conflicts: " + conflictPaths);
...
//            System.out.println(treeWalk.getPathString() + "\t" + treeWalk.getObjectId(0).getName());
    ...
    public static boolean hasConflicts(File project) throws  Exception {
        Git git = GitUtils.getGit(project);
